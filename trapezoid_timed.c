 ...
 *
 * This program uses the trapezoidal rule to approximate the area between a
   hardcoded function ( doulbe f(double x) ) that the user is free to change.
   The user must enter the left-endpoint, right-endpoint, and the number of
   trapezoids used for the approximation.

   The time it takes for the serial and parallel computations will be recorded
   and outputted at the end of the program. The parallel code will uses a
   a mutex to synchronize and the function will be ran three times on parallel
   using 2, 4, and 8 threads. The speedup and efficiency will be calculated
   and printed at the end of the function.
 * @details 
 *
 ...
 *     
 *    gcc -g -Wall -o trapezoid_timed trapezoid_timed.c -lpthread
 *
 * @par Usage:
   @verbatim 
   c:\> trapezoid_timed 
